...
@FragmentDescriptor("area-config-fragment.xml")
public class AreaConfigFragment extends Fragment<VerticalLayout> implements ChartConfigFragment {

    @Autowired private ObjectMapper objectMapper;
    @ViewComponent private InstanceContainer<KeyValueEntity> areaSettingsDc;
    ...
    public void onReady(ReadyEvent event) {
        ...
        entity.setValue("seriesLimit", 10); // Mặc định 10 đường
        entity.setValue("rowLimit", 1000);  // Mặc định 1000 điểm
        entity.setValue("isStacked", true); // Mặc định là Stack (giống Superset)
        areaSettingsDc.setItem(entity);
    ...
    public ObjectNode getConfigurationJson() {
        ...
        KeyValueEntity entity = areaSettingsDc.getItem();
        ...
        node.put("isStacked", (Boolean) entity.getValue("isStacked"));
    ...
    public void setConfigurationJson(JsonNode node) {
        ...
        entity.setValue("seriesLimit", node.path("seriesLimit").asInt(10));
        entity.setValue("rowLimit", node.path("rowLimit").asInt(1000));
        entity.setValue("isStacked", node.path("isStacked").asBoolean(true));
